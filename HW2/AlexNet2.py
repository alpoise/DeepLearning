...
def conv(input,fh,fw,n_out,name):
    ...
    with tf.name_scope(name) as scope:
        ...
        biases = tf.get_variable(scope+'b',shape=[n_out])
...
def full_connected_layer(input,n_in,n_out,name):
    with tf.name_scope(name) as scope:
        ...
        fl  = tf.nn.l2_loss(filter)+tf.nn.l2_loss(biases)
        return [fc,fl]
    ...
    def __init__(
      self, num_classes,l2_reg_lambda=0.01):
        ...
        with tf.name_scope("dense"):
            ...
            self.fc1,self.loss_fc1 = full_connected_layer(self.h_pool_flat,n_in=self.dim,n_out = 4096,name = "fc1")
            self.fc2,self.loss_fc2 = full_connected_layer(self.fc1,n_in = 4096,n_out = 1000,name = "fc2")
            self.out,self.loss_fc3 = full_connected_layer(self.fc2,n_in = 1000,n_out = 17,name = "fc3")
            
            l2_loss += self.loss_fc1 + self.loss_fc2 + self.loss_fc3
